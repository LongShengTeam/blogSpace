...
public class KaoqinController {
    ...
    private static List<OABean> sendRequest(Long hrmId, Integer offset) {
        ...
        //本月7
        //上月 6
        String lastMonth = DateUtil.date().offset(DateField.MONTH, offset-1).toString("yyyy-MM");
        //上上月 5
        ...
        paramMap.put("beginDate", lastMonth + "-01");
        //下月8
        String nextMonth = DateUtil.date().offset(DateField.MONTH, offset + 1).toString("yyyy-MM");
        paramMap.put("endDate", nextMonth + "-01");
        ...
        params.keySet().stream().sorted().forEach(key -> {
            if (key.compareTo(lastMonth + "-01") < 0) {
                params.remove(key);
            }
        });
